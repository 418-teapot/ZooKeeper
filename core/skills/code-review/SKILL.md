...
1. 收集两个 Eagle 原始输出中的全部 Must Fix / Should Fix / Could Fix 发现，逐条标注来源（Eagle 1 / Eagle 2 / 两者）
2. 对指向同一代码位置、同一根因的发现做去重合并，合并后保留一条，并在该条上标明所有来源（如「Eagle 1 + Eagle 2」）
3. 验证：每个 Eagle 的每条发现要么作为独立条目出现在清单中，要么被明确合并到另一条并标注来源 —— 不允许任何一条发现既不独立列出、又无合并记录
4. 已修复的发现也必须保留在清单中，标注为「已修复」并附修复说明；不得因已修复而从清单删除
5. 不得以"超范围"、"既有行为"、"无行为影响"、"显然不重要"为由省略任何发现；这些只能作为处理建议附注，不能作为不报告的理由

去重的目的是避免同一问题重复列出损害可读性，不是裁剪手段。若不确定两条发现是否同一根因，偏向保留为独立条目而非合并。
...
| 检查项 | 标准 |
|--------|------|
| **完整性** | 是否审查了所有变更文件？两个 Eagle 是否都已完成（含重试）？是否每个 Eagle 的每条发现都独立列出或被标注来源合并，无遗漏、无隐式丢弃？是否每条发现都有文件定位？ |
...
| **简洁性** | 报告是否避免了废话和重复？ |
| **无裁剪** | 是否所有 Eagle 发现（含已修复项）都已列入聚合清单，未以超范围/既有行为/已修复为由删除？ |
