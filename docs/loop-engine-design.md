...
本文档记录把 `core/continuation` 泛化为通用 loop 引擎的设计。设计目标：未来每种自主循环（debug、research 等）是一个**可插拔策略（controller）**，引擎内核只有一份。
...
### 3.1 引擎职责（core/continuation）

1. **停稳过滤**：未停稳（`awaiting-input`/`aborted`）时不询问任何策略，记 `not-settled`；
2. **额度联锁**：额度用尽不询问策略，记 `budget-exhausted`；额度状态由引擎持有，暴露记账/重置接口；
3. **询问**：把 `{sessionID, observations}` 逐个交给已注册策略，逐策略崩溃隔离；
4. **汇聚**：首个 `wake` 胜出（沿用现有 runner 语义）；
5. **执行**：先扣额度，再投递 wake 文本；
6. **观测**：引擎原因与策略原因都记入日志。
...
- 删除双宿主平行的 `continuationUsed`（`src/opencode.ts:390`）/ `remindersUsed`（`src/pi.ts:1574`）Map，额度状态移入引擎；
- 引擎暴露记账与重置接口；opencode 的真实用户消息回显区分、pi 的 `before_agent_start`/`session_start` 重置点与 `REMINDER_SESSIONS_CAP` LRU 淘汰语义随记账迁入；
- 配置 `[zoo.continuation].max_reminders` 不变。

### 4.3 行为差异（如实记录，均已接受）

1. 现状 budget 门排在 no-progress 之后；重构后联锁先于策略——"额度尽 + 无进展"时日志原因从 `no-progress` 变为 `budget-exhausted`（行为同为不唤醒，真因更准确）；
...
| 4 | 第一性原理方案 | 已接受（§2 本质回答 + §3 架构 + §3.4 检查） |
...
- 不做 per-controller 预算（沿用单一全局 `max_reminders`）；
- 不改配置项、不改 wake 文案、不改汇聚语义（首个 wake 胜出）。
...
- **per-controller 预算与多 controller 仲裁**：当前为全局预算 + 首个 wake 胜出，多 controller 并存时需重审；
