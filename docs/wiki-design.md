...
**实现架构：** 工作流从系统 prompt 剥离为单一 kiwi-distill skill，kiwi 按需加载。矛盾检测（#11）不建独立 skill，而是作为 kiwi-distill 的 Phase 4.7 Claim Reconciliation 的一个分支集成在蒸馏流程中——kiwi 在对比新旧声明时统一产出三个结果：取代（supersede）、矛盾（contradiction）、印证（validation），共享 claim 提取逻辑：

```
kiwi 系统 prompt（~40 行）          kiwi 技能
  ├── Role: 只读知识分析引擎    ←──  kiwi-distill（蒸馏工作流，✅ 已实现，含统一 Claim Reconciliation：取代 / 矛盾 / 印证）
  └── Contract: 不写文件
```
...
三阶段分离：
1. **发现**（半自动）— 集成于 kiwi-distill Phase 4.7 Claim Reconciliation：蒸馏过程中 kiwi 对比新源声明与已有页面声明，当发现具体声明冲突且任一声明不明确构成取代时，输出矛盾提议（只问"是否不一致"，不问"谁对"）。**无独立预筛选 scanner**——矛盾发现依赖蒸馏过程中的语义理解，图拓扑机械预筛不足以替代
...
| 矛盾候选发现（kiwi-distill Phase 4.7 Claim Reconciliation 语义对比） | ⚠️ 半自动 | 蒸馏时对比新旧声明发现矛盾，人不裁决；`zwiki contradictions apply` 半自动写入 |
