/**
 * Tests for v1 fold-view materialization (`apply-view.ts`).
 *
 * Two layers:
 *
 * 1. **Shape** — `materializeSummary` and `applyView` output satisfy
 *    the v1 schema: `info.role` / `info.id` / `info.synthetic` and a
 *    single text part.
 * 2. **Injection provenance** — line-number prefixes land only on
 *    `isInjectableRegion`-true regions (text-derived content and
 ...
 *    strip→inject is idempotent across rounds.
 */
...
import { applyView, materializeSummary } from "./apply-view.js";
...
// ---------------------------------------------------------------------------
// Shape: applyView output keeps the v1 schema complete
// ---------------------------------------------------------------------------

describe("applyView shape", () => {
  it("every output message keeps the v1 schema complete", () => {
    ...
    applyView(msgs, lens, items, state);
...
describe("applyView injection", () => {
  ...
  it("strip→inject is idempotent across rounds", () => {
    const msgs = noBlockFixture();
    const lens = history(msgs);
    const state: SessionState = { blocks: new Map(), marks: new Map() };
    const { items } = fold(lens, state);
    ...
    applyView(msgs, lens, items, state);
    const afterFirst = msgs.map((m) =>
      JSON.stringify(m.parts?.map((p) => (p as { text?: string }).text ?? "")),
    );

    applyView(msgs, lens, items, state);
    const afterSecond = msgs.map((m) =>
      JSON.stringify(m.parts?.map((p) => (p as { text?: string }).text ?? "")),
    );
