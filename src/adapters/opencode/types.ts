 ...
 * they belong here — not in the framework-agnostic core.
 *
 * Framework-agnostic estimation lives in `src/core/context/measure.ts`
 * (the pure CJK-aware token estimator is imported from there, never
 * duplicated).  The report data types (`ContextReport`,
 * `ContextCategoryBreakdown`) are declared in
 * `src/core/context/context-report.ts`, the display module that also
 * formats them.
 *
 * @module
 */

import type { ContextReport } from "../../core/context/context-report.js";
 ...
 * identifier in the OpenCode SDK (use `getCallId` from this module).
 */
 ...
 *
 * Text parts are estimated via `estimateTokenCount`; tool parts via
...
export function estimateMessageHeuristic(
  ...
  for (const part of parts) {
    ...
    if (extPart.type === "tool" && extPart.state) {
      ...
      tokens += estimateTokenCount(extPart.text);
    }
  }
  return tokens;
}
...
// ---------------------------------------------------------------------------
// Shared scan result and helpers
// ---------------------------------------------------------------------------
      ...
      if (msg?.parts) {
        for (const part of msg.parts) {
          ...
          if (extPart.type !== "tool" && extPart.text) {
            assistantTokens += estimateTokenCount(extPart.text);
          }
        }
      }
    }
