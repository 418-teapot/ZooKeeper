...
├── installer/               # 安装逻辑包（envfile 解析 / variants 校验 / thinking 字段翻译 /
│   │                        #   opencode·pi 配置生成 / jsonio IO helper / output 终端输出；
│   │                        #   单元测试在 installer/tests/）
...
- **`install.py`** — 安装脚本入口，读取 config.toml + .env → 用 `shutil.which` 检测 opencode/pi 是否安装 → 按检测结果生成：OpenCode 的 `~/.config/opencode/opencode.json`、pi 的 `~/.pi/agent/settings.json`（每次安装完整重建，只含 extensions + 从 `[defaults].model`（`{env:ZOO_WHALE_MODEL}`，`Provider/model` 格式）拆出的 defaultProvider/defaultModel，不读取旧文件）+ `~/.pi/agent/models.json`（provider 转换：明文 apiKey、baseUrl 对 anthropic-messages 去 `/v1`、cost 补全四字段、idempotent prune 残留）+ `~/.pi/agent/agents.json`（每次安装完整重建，解析各 `[agent.<name>].model` 的 `{env:}` 占位符为 `Provider/model` 明文，per-agent 映射物化给 pi 扩展运行时读取；无 model 或 env 缺失的 agent 缺席，不读取旧文件）。provider 跳过 warn 统一打一次。三阶段对称打印（备份/生成/验证/安装完成）。只依赖 Python 标准库。
- **`installer/`** — install.py 委托的安装逻辑包：`envfile.py`（.env 解析 + `{env:VAR}` 递归解析 + 凭据缺失条目剔除）、`variants.py`（`[zoo.variants]` 全局/按 agent 双通道校验收集）、`thinking.py`（模型级 `thinking = "high"|"max"|"none"` 语义字段的校验与读取，由 opencode/pi 两个生成器各自翻译：OpenCode 写模型 `options.effort`（@ai-sdk/anthropic，由 AI SDK 合成 `output_config.effort` 发给网关）或 `options.reasoning_effort`（@ai-sdk/openai*），pi 写 `compat.forceAdaptiveThinking`，两侧都会剔除 `thinking` 键）、`opencode.py`（mode profile 解析 + opencode.json 组装）、`pi.py`（provider → models.json 转换 + per-agent → agents.json 解析）、`jsonio.py`（JSON 读写 helper）、`output.py`（终端输出）。单元测试位于 `installer/tests/`。
