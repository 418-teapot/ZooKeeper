...
> autoresearch 中的人类可编辑 agent 指令文件。本质是一个"轻量级 skill"：定义实验目标、约束条件、评估标准、行为准则和停止条件。Agent 通过阅读此文件理解自己的角色和任务边界。

## Overview

program.md 是 [autoresearch](sources/notes/autoresearch.md) 的"组织代码"——不是模型代码，而是定义实验如何组织的自然语言指令。它由人类编写、迭代，由 agent 读取、执行。约 114 行。

## Role

- **实验目标** — "get the lowest val_bpb"
- **权限边界** — 可修改 [train.py](entities/autoresearch-train-py.md)，不可修改 [prepare.py](entities/autoresearch-prepare-py.md) 和依赖
...
- [自主实验循环](concepts/autonomous-experiment-loop.md) — 本文件定义循环的规则
- [NPC 式分工](concepts/npc.md) — 本文件是人类编写的"NPC 脚本"
- [简约准则](concepts/simplicity-criterion.md) — 本文件定义该准则
- [train.py](entities/autoresearch-train-py.md) — 本文件约束 agent 如何修改 train.py
...
> **待确认：** 当前 [program.md](entities/autoresearch-program-md.md) 为单 agent 设计。多 agent 协作时可能需要扩展为多个 skill 文件或增加协调章节。
