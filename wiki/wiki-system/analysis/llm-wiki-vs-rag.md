...
> Karpathy 在文章中通过对比 RAG 来定义 [LLM Wiki](wiki-system/sources/notes/llm-wiki-karpathy.md) 模式。这个对比不是技术优劣的评判，而是**两种截然不同的知识管理哲学**：RAG 走"实时检索→综合→丢弃"路径；LLM Wiki 走"预编译→存储→[复利](wiki-system/concepts/compounding-knowledge.md)"路径。
...
| **知识增长** | 静态 — 增加文档不自动更新已有知识 | 动态 — 每次 [ingest](wiki-system/concepts/wiki-ingest-workflow.md) 涟漪式更新相关页面 |
