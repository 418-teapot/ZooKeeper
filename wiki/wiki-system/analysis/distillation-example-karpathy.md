...
> 本文以 Karpathy 的 [LLM Wiki](wiki-system/sources/notes/llm-wiki-karpathy.md) gist 为例，完整展示一次蒸馏的决策过程：[复利知识](wiki-system/concepts/compounding-knowledge.md)如何从原文提炼、三个操作（[ingest](wiki-system/concepts/wiki-ingest-workflow.md) / [query→synthesis](wiki-system/concepts/wiki-query-synthesis.md) / [lint](wiki-system/concepts/wiki-health-check.md)）如何拆为独立概念页、跨页结构如何设计，以及[LLM Wiki vs RAG](wiki-system/analysis/llm-wiki-vs-rag.md)对比页如何从概念页衍生。是一份"蒸馏过程的蒸馏"——meta 级别的示例，供后续蒸馏任务参考。
...
#### 第 1 段：核心主张 → `wiki-system/concepts/compounding-knowledge.md`
...
**合并融入 `wiki-system/concepts/compounding-knowledge.md` 和 `overview.md`。** 三层架构（raw / wiki / schema）没有独立成页，因为与 ZooKeeper 自身的架构高度重叠——config.toml 对应 schema，src/ 和 prompts/ 对应 wiki。独立成页会产生大量与已有知识冗余的内容
...
| 原文子节 | 蒸馏为 | 原因 |
|----------|--------|------|
| Ingest | `wiki-system/concepts/wiki-ingest-workflow.md` | 流程步骤 + 设计考量，是独立知识单元 |
| Query | `wiki-system/concepts/wiki-query-synthesis.md` | "回答归档"是 Karpathy 文章中最独特的洞察，值得独立成页 |
| Lint | `wiki-system/concepts/wiki-health-check.md` | 检查维度 + 与工具分工，是独立知识单元 |
...
```
wiki-system/sources/notes/llm-wiki-karpathy.md        ← 原始材料记录（所有页面的上游）
         │
         ▼
wiki-system/concepts/compounding-knowledge.md         ← 核心概念（被所有其他页面引用）
    │              │              │
    ▼              ▼              ▼
wiki-system/concepts/        wiki-system/concepts/       wiki-system/concepts/
wiki-ingest-     wiki-query-     wiki-health-
workflow.md     synthesis.md    check.md
    │              │
    └──────┬───────┘
           ▼
wiki-system/analysis/llm-wiki-vs-rag.md              ← 综合对比（消费 concepts 而生产 analysis）
```
...
- [LLM Wiki — 用 LLM 构建个人知识库的模式](wiki-system/sources/notes/llm-wiki-karpathy.md) — 被蒸馏的原始材料
- [复利知识 — 持久化知识库的核心价值](wiki-system/concepts/compounding-knowledge.md) — 蒸馏出的核心概念页面之一
- [Wiki Ingest 工作流 — 源材料的增量整合](wiki-system/concepts/wiki-ingest-workflow.md) — 蒸馏出的操作页面之一
- [Query → Synthesis → 归档 — 查询即知识生产](wiki-system/concepts/wiki-query-synthesis.md) — 蒸馏出的操作页面之一
- [Wiki 健康检查 — 知识库的持续质量维护](wiki-system/concepts/wiki-health-check.md) — 蒸馏出的操作页面之一
- [LLM Wiki vs RAG — 两种知识管理范式的对比](wiki-system/analysis/llm-wiki-vs-rag.md) — 蒸馏出的综合对比页面
