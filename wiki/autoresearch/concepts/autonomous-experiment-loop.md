...
自主实验循环是 [autoresearch](autoresearch/sources/notes/autoresearch.md) 项目的核心机制。它让 AI agent（如 Claude、Codex）在无人值守的情况下，整夜持续进行 LLM 训练实验。每次循环约 5 分钟，一个睡眠周期可完成约 100 次实验。

oh-my-pi 的 [autoresearch 扩展循环](autoresearch/concepts/autoresearch-extension-loop.md)在此基础上增加了插件基础设施、SQLite 持久化和自动恢复机制。
...
1. **读取上下文** — agent 阅读 [program.md](autoresearch/entities/autoresearch-program-md.md) 获取实验指令和约束
2. **提出假设** — 基于历史结果和代码理解，决定修改方向
3. **修改代码** — 直接编辑 [train.py](autoresearch/entities/autoresearch-train-py.md)（唯一可修改文件，遵循[单文件修改原则](autoresearch/concepts/single-file-modification.md)）
4. **提交版本** — `git commit` 记录实验版本，每次提交作为[实验版本管理](autoresearch/concepts/experiment-versioning.md)的一个节点
