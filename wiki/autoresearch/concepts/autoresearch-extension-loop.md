...
每次迭代遵循固定操作序列，其版本控制机制与[实验版本管理](autoresearch/concepts/experiment-versioning.md)一致：
...
- `autoresearch/sources/rfc/autoresearch-design.md` §5 核心循环流程
- `autoresearch/sources/rfc/autoresearch-design.md` §12 插件 Hook 集成

## Notes

> **待确认：** ZooKeeper 基于 OpenCode SDK，当前无 `agent_end` hook 和 `sendMessage` API，auto-resume 机制需重新设计（参见 [移植路线图](autoresearch/analysis/autoresearch-porting-roadmap.md)）。
