...
train.py 是 [autoresearch](autoresearch/sources/notes/autoresearch.md) 的核心文件，约 630 行。它既是模型定义文件，也是训练脚本。Agent 通过修改此文件来尝试不同的架构、优化器和超参，修改规则由 [program.md](autoresearch/entities/autoresearch-program-md.md) 定义。
...
- **输入** — 由 [prepare.py](autoresearch/entities/autoresearch-prepare-py.md) 提供的数据加载器（`make_dataloader`）
