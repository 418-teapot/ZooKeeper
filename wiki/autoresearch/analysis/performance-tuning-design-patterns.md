...
通过对 TuneAgent、BYOS、KForge、Arbor、GPU Forecasters、Kernel Foundry、NPU Agent Skill 等系统的调研，提炼出 6 个可复用的设计模式。这些模式可指导 [ZooKeeper 移植路线图](autoresearch/analysis/autoresearch-porting-roadmap.md)中的内核调优 agent 规划。当前领域空白：没有 LLM agent 消费 `perf` 输出 → 定位热点 → 建议代码级优化的完整开源流水线。
...
- `autoresearch/sources/rfc/autoresearch-design.md` §2 行业调研与相关工作
