...
> 将 oh-my-pi 的 [autoresearch 扩展](concepts/autoresearch-extension-loop.md)移植到 ZooKeeper（OpenCode 插件）的三阶段计划。最大障碍是 session 管理差异：Pi 原生支持深度生命周期 hook 和自定义 entry 写入，OpenCode SDK 当前仅提供只读 session 访问。
...
- [autoresearch 扩展循环](concepts/autoresearch-extension-loop.md) — 被移植的核心机制
- [Agent/Skill/Plugin 判断框架](analysis/agent-skill-plugin-framework.md) — 移植的架构设计基础
- [性能调优设计模式](analysis/performance-tuning-design-patterns.md) — Phase 2 特化的参考模式
...
- `sources/rfc/autoresearch-design.md` §19 ZooKeeper 实现路线
- `sources/rfc/autoresearch-design.md` §19.3 Session 管理差异深度分析
