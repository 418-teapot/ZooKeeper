...
> 不依赖文本内容，仅从页面间的拓扑结构推断"哪些页面应该互连但未连"。作为 Wiki 健康检查中"缺失交叉引用"检测的互补路径。

## Overview

Wiki 已有的锚文本挖掘（见 [Wiki 健康检查](concepts/wiki-health-check.md) 中的 `check_missing_inline_links`）依赖已建立的链接映射表发现缺失内联链接，对新页面或冷门概念覆盖不足。图链接预测提供一条纯拓扑路径：将 wiki 视为有向图（节点 = 页面，边 = `related` 声明 + 正文内联链接），通过邻居集合的交集和路径结构推断缺失边。
...
- [Wiki 健康检查 — 知识库的持续质量维护](concepts/wiki-health-check.md) — 图链接预测作为"缺失交叉引用"检测的拓扑互补路径
- [复利知识 — 持久化知识库的核心价值](concepts/compounding-knowledge.md) — 交叉引用密度是 wiki 产生 n² 级关联价值的前提
- [LLM Wiki vs RAG — 两种知识管理范式的对比](analysis/llm-wiki-vs-rag.md) — 超线性价值依赖交叉引用的完整性
