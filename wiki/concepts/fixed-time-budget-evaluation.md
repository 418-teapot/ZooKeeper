...
> 以固定 wall-clock 时间（5 分钟）作为实验控制变量，以 `val_bpb`（validation bits per byte）作为 vocab-independent 评估指标。两者结合使得不同架构、超参、模型尺寸在同等时间成本下可直接比较。

## Overview

固定时间预算评估是 [autoresearch](sources/notes/autoresearch.md) 的核心评估设计。它解决了"不同实验如何公平比较"的问题——不是比较固定 epoch 数，而是比较在相同 wall-clock 时间内谁的表现更好。
...
- **与 [prepare.py](entities/autoresearch-prepare-py.md) 绑定** — 评估函数固定，agent 不可修改
...
- [自主实验循环](concepts/autonomous-experiment-loop.md) — 本概念是循环的评估环节
- [单文件修改原则](concepts/single-file-modification.md) — 评估指标固定是"单文件"约束的一部分（[prepare.py](entities/autoresearch-prepare-py.md) 不可改）
- [prepare.py](entities/autoresearch-prepare-py.md) — 包含固定的 `evaluate_bpb` 实现
...
- karpathy/autoresearch [prepare.py](entities/autoresearch-prepare-py.md) 中的 `evaluate_bpb` 实现
- [program.md](entities/autoresearch-program-md.md) 中的评估说明
