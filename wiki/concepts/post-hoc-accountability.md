...
> 不阻止 agent 的越界编辑，而是在实验记录时捕获偏差并要求合理性说明。通过透明度而非硬限制来管理 agent 的自主性，在灵活性与可审计性之间取得平衡。

## Overview

后验问责制是 [autoresearch 扩展](concepts/autoresearch-extension-loop.md)的权限设计哲学。与"编辑防护"（阻止 agent 修改特定文件）不同，它允许 agent 修改任何文件，但要求对超出 scope 的修改负责。
...
- [autoresearch 扩展循环](concepts/autoresearch-extension-loop.md) — 问责制是循环的权限管理基础
- [单文件修改原则](concepts/single-file-modification.md) — Karpathy 原始方案使用硬限制，扩展改用问责制
...
- `sources/rfc/autoresearch-design.md` §15.1 后验问责 vs 编辑防护
- `sources/rfc/autoresearch-design.md` §11.3 log_experiment 工具规范
