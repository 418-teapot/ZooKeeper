...
> 使用 git 分支和 commit 作为实验的版本控制系统：每个 session 一个独立分支，每次实验一个 commit，失败则 reset。`results.tsv` 作为实验日志（不纳入 git），记录 commit hash、指标、状态和描述。

## Overview

实验版本管理是 [autoresearch](sources/notes/autoresearch.md) 的纯 git 实验跟踪方案。它不需要 Weights & Biases 或 TensorBoard 等外部服务，仅依赖 git 的 branch/commit/reset 机制。
...
- **每个实验 = 一个 commit** — 包含完整的 [train.py](entities/autoresearch-train-py.md) 修改
...
- [自主实验循环](concepts/autonomous-experiment-loop.md) — 版本管理是循环的持久化机制
- [train.py](entities/autoresearch-train-py.md) — 被版本控制的文件
...
- [program.md](entities/autoresearch-program-md.md) 中的 "The experiment loop" 和 "Logging results" 段落
