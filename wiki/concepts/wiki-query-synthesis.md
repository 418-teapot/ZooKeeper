...
> Karpathy 的一个关键洞察：**好的回答不该消失在聊天历史里。** 一次有价值的查询（对比分析、连接发现、综合理解）的产出本身是知识，应该像[摄入源材料](concepts/wiki-ingest-workflow.md)一样归档到 wiki 中。这让查询也变成[知识积累](concepts/compounding-knowledge.md)的渠道——不只是消费知识库，也在生产知识。
...
- [复利知识 — 持久化知识库的核心价值](concepts/compounding-knowledge.md) — 查询归档是复利机制的第二条增长曲线
- [Wiki Ingest 工作流 — 源材料的增量整合](concepts/wiki-ingest-workflow.md) — 归档执行流程与 ingest 类似
